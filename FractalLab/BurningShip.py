"""BurningShip.py: Implementation of the BurningShip Set as a subclass of Fractal."""
...
class BurningShip(Fractal):
    ...
    def __init__(self, width: int, height: int, xmin: float,
        ...
        """Constructor for the BurningShip Set fractal class.
    ...
    def iterate(self) -> array:
        """Performs one iteration of the BurningShip set algorithm.

        Returns:
            array: A 2D NumPy array representing the state of the fractal after the iteration."""
        mask: array = abs(self.z) < 2
        self.z[mask] = (abs(self.z[mask].real) + 1j * abs(self.z[mask].imag)) ** 2 + self.constant[mask]
        self.fractal += mask
        return self.fractal
